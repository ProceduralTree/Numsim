...
void step(PDESystem& system, double time)
{
  ...
  parallel_broadcast(calculate_F, system.u.range, system);
  parallel_broadcast(calculate_G, system.v.range, system);

  parallel_broadcast(calculate_pressure_rhs, system.p.range, system);

  solve_pressure(system);

  parallel_broadcast(update_u, system.u.range, system);
