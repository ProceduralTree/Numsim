...
std::stringstream
set_filename(const vtkSmartPointer<vtkXMLImageDataWriter> writer) {
    ...
    writer_->SetFileName(fileName.str().c_str());
};

void initialize_dataset(const PDESystem &system) {
    ...
    dataSet->SetDimensions(
        system.size_x + 1, system.size + 1,
        1); // we want to have points at each corner of each cell

    return dataSet;
};

void write_vtk(const PDESystem &system) {
    {
        set_filename(vtkWriter_);
        ...
        int index = 0; // index for the vtk data structure, will be incremented
                       // in the inner loop
        for (int j = 0; j < nCells[1] + 1; j++) {
            for (int i = 0; i < nCells[0] + 1; i++, index++) {
                ...
                arrayPressure->SetValue(index, interpolateAt(system.p, x, y));
            }
        }
